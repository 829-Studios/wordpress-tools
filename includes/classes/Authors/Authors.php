...
namespace WordPressTools\Authors;
...
class Authors {
	...
	/**
	 * Check to see if author archive page should be disabled for 829 Studios user accounts
	 */
	public function maybe_disable_author_archive() {
		...
		// If the query object doesn't have a user e-mail address or the filter is allowing 829 Studios authors, bail
		if ( ! empty( $filtered_domains ) ||
			empty( $author->data->user_email ) ||
			true === apply_filters( 'wpt_allow_829author_pages', false ) ) {

			return;

		}

		// E-mail addresses containing 829llc.com will be filtered out on the front-end
		if ( false !== stripos( $author->data->user_email, '829llc.com' ) ) {
			$is_author_disabled = true;
		}

		if ( true === $is_author_disabled ) {
			\wp_safe_redirect( home_url(), '301', '829 Studios WordPress Tools' );
			exit();
		}
	}
}
