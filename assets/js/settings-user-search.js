...
( function () {
	...
	function getSelectedIds( idsContainer ) {
		return Array.from(
			idsContainer.querySelectorAll( 'input[type="hidden"]' )
		).map( function ( input ) {
	...
	function attachTagRemoveHandlers( tagsContainer, idsContainer ) {
		tagsContainer
			.querySelectorAll( '.wpt-user-tag-remove' )
	...
	function initAutocomplete( input, options ) {
		...
		function setActive( index ) {
			listEl
				.querySelectorAll( '.wpt-autocomplete-item' )
				.forEach( function ( li ) {
		...
		input.addEventListener( 'input', function () {
			...
			options.source( { term }, renderItems );
		} );
		...
		input.addEventListener( 'keydown', function ( e ) {
			...
			if ( 'ArrowDown' === e.key ) {
				...
				setActive(
					Math.min( activeIndex + 1, currentItems.length - 1 )
				);
	...
	function initUserSearch( wrapper ) {
		...
		const input = wrapper.querySelector( '.wpt-user-search-input' );
		const tagsContainer = wrapper.querySelector( '.wpt-user-tags' );
		const idsContainer = wrapper.querySelector( '.wpt-user-ids' );
	...
	function initRestrictionToggle( name ) {
		const radios = document.querySelectorAll(
			'input[name="wpt_settings[' + name + ']"]'
		);

		function updateVisibility( value ) {
			document
				.querySelectorAll( '[data-restriction="' + name + '"]' )
				.forEach( function ( el ) {
	...
	document.addEventListener( 'DOMContentLoaded', function () {
		document
			.querySelectorAll( '.wpt-user-search' )
